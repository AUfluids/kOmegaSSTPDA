...
def get_figure_size(ncols: int, nrows: int) -> Tuple[float, float]:
    ...
    Uses the multi plot (1×3) as the base reference size, scaling proportionally.
    ...
    """
    fig_width = BASE_WIDTH_PER_COL * ncols
...
def create_comparison_contour_plot(
    ...
    # Add colorbars (one per row, vertical, on the right side)
    # Row 0: Velocity
    cbar1 = fig.colorbar(
        cs, cmap='turbo', pad=0.02, orientation='vertical',
        norm=norm1, ax=ax[0, :], aspect=20, shrink=1.0
    )
    # Row 1: Velocity error
    cbar2 = fig.colorbar(
        csUst, cmap='RdBu_r', pad=0.02, orientation='vertical',
        norm=norm2, ax=ax[1, :], aspect=20, shrink=1.0
    )
    # Row 2: Vorticity error
    cbar3 = fig.colorbar(
        csVort, cmap='PuOr_r', pad=0.02, orientation='vertical',
    ...
    # Apply tight layout with improved vertical spacing
    fig.tight_layout(pad=1.0, h_pad=0.3, w_pad=0.5)
